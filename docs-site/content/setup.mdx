...
Docker is the fastest way to get Open Agent running on any machine (including macOS).
...
OpenCode server is optional for mission execution. Open Agent runs OpenCode per-workspace via the CLI. Install the server if you want centralized provider/auth management or health checks.
