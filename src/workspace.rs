...
async fn write_claudecode_config(
    ...
    // For container workspaces, add permissive settings to avoid permission prompts
    // since the environment is isolated
    let settings = match workspace_type {
        WorkspaceType::Chroot => json!({
    ...
    if let Some(skills) = skill_contents {
        ...
        if skills.is_empty() {
            ...
            // Add container workspace instructions
            if matches!(workspace_type, WorkspaceType::Chroot) {
